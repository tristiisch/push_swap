...
void	ft_print_print_result(t_stack_master *best_stack_master)
{
	if (best_stack_master == NULL)
		ft_error("Aucune données concernant le meilleur algo.");
	else
	{
		printf("\033[32mLe meilleur algo pour %d chiffres est le n°%d" \
			" : %d\033[0m\n", best_stack_master->a->capacity,
			best_stack_master->algo_version, best_stack_master->instructions);
...
void	ft_test_all_algos(t_stack_master *stack_master, int argc, char **argv)
{
	...
	while (i <= max_algo)
	{
		...
		if (best_stack_master == NULL
			|| best_stack_master->instructions > stack_master->instructions)
	...
	ft_print_print_result(best_stack_master);
}
